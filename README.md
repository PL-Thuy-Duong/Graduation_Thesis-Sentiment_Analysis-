...
Stopword removal
Lemmatization
Filtering irrelevant or non-emotional content
...
Valid words extracted from captions
26 emotion categories
Mood tags
Related words derived from SenticNet 5 and WordNet
Each valid word retrieves:
5 related words from SenticNet / WordNet
Each related word further retrieves additional semantic connections
Edge Construction
Word–Emotion edges: weighted using co-occurrence probability
Word–Word edges: based on normalized co-occurrence frequency
Word–MoodTag edges: derived from SenticNet pleasantness values
Related-word connections: weighted using polarity values from SenticNet
